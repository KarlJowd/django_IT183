...
def attendance_report(request):
    """
    Attendance summary report - EXTRA FEATURE (+20 points)
    Shows statistics for each student including:
    ...
    if classroom_id:
        attendance_filter &= Q(classroom_id=classroom_id)
    if date_from:
    ...
    for student in students:
        records = AttendanceRecord.objects.filter(student=student).filter(attendance_filter)
